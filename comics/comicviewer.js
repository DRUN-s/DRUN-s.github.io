$(function () {
  ...
  const tomeData = [
    { 
      ...
      title: Object.fromEntries(Object.keys(translations).map(lang => [lang, translations[lang].tf00Title])),
      pages: 20 
    },
    { key: "tf01", 
      folder: "tf01_ring-of-fired", 
      baseName: "tf01_ring_of_fired", 
      title: Object.fromEntries(Object.keys(translations).map(lang => [lang, translations[lang].tf01Title])),
      pages: 68 
    },
    { key: "tf02", 
      folder: "tf02_unhappy-returns", 
      baseName: "tf02_unhappy_returns", 
      title: Object.fromEntries(Object.keys(translations).map(lang => [lang, translations[lang].tf02Title])),
      pages: 75 
    },
    { key: "tf03", 
    ...
    { key: "tf04", 
      ...
      title: Object.fromEntries(Object.keys(translations).map(lang => [lang, translations[lang].tf04Title])),
        pages: 113 },
    { key: "tf05", 
      folder: "tf05_old-wounds", 
      baseName: "tf05_old_wounds", 
      title: Object.fromEntries(Object.keys(translations).map(lang => [lang, translations[lang].tf05Title])),
      pages: 79 
    },
    ...
    { key: "tf06", 
      ...
      title: Object.fromEntries(Object.keys(translations).map(lang => [lang, translations[lang].tf06Title])),
      pages: 274 
    },
    ...
    { key: "tf07", 
      ...
      title: Object.fromEntries(Object.keys(translations).map(lang => [lang, translations[lang].tf07Title])),
      pages: 330 
    }
  ];

  function findTome(key) { return tomeData.find(t => t.key === key); }
  function getNextTome(currentKey) { const index = tomeData.findIndex(t => t.key === currentKey); return tomeData[index + 1] || null; }
  ...
  function preloadImages({ lang, tomeKey, pageNum }) {
    ...
    preloadRange.forEach(offset => {
      ...
      const img = new Image(); img.src = `${tome.folder}/${lang}/${tome.baseName}-${targetPage}.jpg`;
      if (lang !== "EN") {
        const imgEn = new Image(); imgEn.src = `${tome.folder}/EN/${tome.baseName}-${targetPage}.jpg`;
      }
    });
  }
  ...
  async function loadPage(data) {
    ...
    if (fallback) {
      alert(`This page hasn't been translated in ${data.lang}. You can help us going faster on the project's discord. Fallback to the original version.`);
      updateHash({ ...data, lang: langUsed }); // Changes the page to english version
    } else if (notFound) {
      alert(`The page ${data.pageNum} from the issue ${data.tomeKey} cannot be found, enjoy this SMPTE test card as a placeholder.`);
    }
  ...
  function changePage(delta) {
    ...
    if (newPage > currentTome.pages) {
      ...
      if (prevTome) {
        data = { lang: data.lang, tomeKey: prevTome.key, pageNum: prevTome.pages };
      } else return;
    } else {
  ...
  else updateHash({ lang: localStorage.getItem("selectedLanguage") || "EN", tomeKey: "tf00", pageNum: 1 });
