document.addEventListener("DOMContentLoaded", function () {
  ...
  const defaultTexts = {}; //here are stocked the default texts in case they weren't added in translation.js
  ...
  let effectiveLang =
    savedLang || (supportedLangs.includes(langCode) ? langCode : "EN");
  ...
  const item = document.querySelector(
    `#select-container li[lang-selection="${effectiveLang}"]`
  );
  if (item && selectedItem) {
    selectedItem.innerHTML =
      item.innerHTML + '<span class="arrow-down"></span>';
  ...
  function checkComicsAvailability(lang) {
    ...
    const comics = Object.fromEntries(
      [
        ...
        {
          dir: `${id}_${slug}`,
          file: `${id}_${slug.replace(/-/g, "_")}-1.jpg`,
        }, //this line converts kebab case (folders) to snake case (files)
      ])
    );
